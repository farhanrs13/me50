...
def minimax_value(board, best_value):
    ...
    player_user = player(board)
    value = float("-inf") if player_user == X else float("inf")

    for action in actions(board):
        latest_value = minimax_value(result(board, action), value)

        if player_user == X:
            if latest_value > best_value:
                return latest_value
            value = max(value, latest_value)

        if player_user == O:
            if latest_value < best_value:
                return latest_value
            value = min(value, latest_value)
